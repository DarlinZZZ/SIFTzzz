...
def BFmatch(ori_p, noise_p, distance, layer):
    '''
    ...
    kp_oriimg = cv2.drawKeypoints(ori_img, tuple(kp_second_ori), cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS,
                                  (51, 163, 236), 4)
    writekp(keypoints_ori, ori_kp_name)
    kp_noiseimg = cv2.drawKeypoints(noise_img, tuple(kp_second_noise), cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS,
                                    (255, 192, 203), 4)
    if len(tuple(kp_second_noise)) == 0:
        print('fail = 0')
    else:
        print('true = ' + str(len(tuple(kp_second_noise))))
        writekp(tuple(kp_second_noise), noise_kp_name)
        bf = cv2.BFMatcher()
        ...
        max2[0] = min1[0] + 64
        max2[1] = min1[1] + 64
        print('通过sift匹配出来的模糊子图在原图的位置：左上坐标和右下坐标')
        print('原图x = ' + str(min1))
        print('原图y = ' + str(max2))
        ...
        result_img = cv2.drawMatchesKnn(ori_img, tuple(kp_second_ori), noise_img, tuple(kp_second_noise), good, None,
                                        flags=2)
        # print(good)
        # print(len(good))
...
if __name__ == '__main__':
    ...
    cut_path = r'./cut/cut1/cut1_1.png'
    BFmatch(ori_path, cut_path, 180, 2)
